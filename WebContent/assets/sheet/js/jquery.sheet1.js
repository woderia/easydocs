/*
 ...
 $Id: jquery.sheet.js 435 2011-11-30 02:23:08Z RobertLeePlummerJr@gmail.com $
    ...
    sheet: function(settings){
        jQuery(this).each(function(){
            ...
            var set = jQuery.extend({
                ...
                urlMenu: "", //local url, for the menu to the left of title
                ...
                fnSave: function(){
                    parent.getSheet().saveSheet();
                }, //fn, default save function, more of a proof of concept
...
jQuery.sheet = {
    createInstance: function(s, I, origParent){ //s = jQuery.sheet settings, I = jQuery.sheet Instance Integer
        var jS = {
            ...
            obj: {//obj = object references
                ...
                autoFiller: function(){
                    return jQuery('#' + jS.id.autoFiller + jS.i);
                },
                barCorner: function(){
                ...
                barLeftMenu: function(){
                    return jQuery('#' + jS.id.barLeftMenu);
                },
                barTop: function(){
                ...
                barTopMenuParent: function(){
                    return jQuery('#' + jS.id.barTopMenuParent);
                },
                barTopMenu: function(){
                    return jQuery('#' + jS.id.barTopMenu);
                },
                cellActive: function(){
                    return jQuery(jS.cellLast.td);
                },
                cellMenu: function(){
                    return jQuery('#' + jS.id.cellMenu);
                },
                ...
                inlineMenu: function(){
                    return jQuery('#' + jS.id.inlineMenu);
                },
            ...
            msg: { /*msg = messages used throught sheet, for easy access to change them for other languages*/
               ...
               /* addRowMulti: "How many rows would you like to add?",
                ...
                merge2: "Merging is not allowed merge 2 currently",
                mergeNoChoose: "Merging is not allowed No selected target",
                ...
                */
                addRowMulti: "加入行数",
                addColumnMulti: "加入列数：",
                newSheet: "表格几行几列?例子: '5x10' 5 列 10 行.",
                openSheet: "确定打开不同页?所有未保存更改即将丢失",
                cellFind: "无结果返回",
                toggleHideRow: "无任何行被选中.",
                toggleHideColumn: "无任何列被选中.",
                merge: "不允许合并第一行的单元格.",
                merge2: "不允许合并",
                mergeNoChoose: "请选择要合并的目标单元格",
                evalError: "错误，作为公式的函数不支持.",
                menuInsertColumnAfter: "在右侧插入列",
                menuInsertColumnBefore: "插入列",
                menuAddColumnEnd: "在最后加入列",
                menuDeleteColumn: "删除列",
                menuInsertRowAfter: "在下方插入行",
                menuInsertRowBefore: "插入行",
                menuAddRowEnd: "在最后加入行",
                menuDeleteRow: "删除行",
                menuAddSheet: "增加页",
                menuInsertKPI: "插入指标...",
                //menuInsertVLook : "插入透视图",
                //menuInsertDIM: "编辑指标",
                menuFillMEA:"派生维度成员...",
                menuCellFresh:"选择维度成员...",
                //menuFillDIM:"填充指标",
                menuClearCon:"清除内容",
                menuformatTar:"设置单元格格式...",
                menuCopy:"复制",
                menuPaste:"粘贴",
                menuCut:"剪切",
                //menuEditGroup: "工具组",
            ...
            kill: function(){ /* For ajax manipulation, kills this instance of sheet entirley */
                ...
                origParent.removeClass(jS.cl.uiParent).html('').removeAttr('sheetInstance');
            ...
            spreadsheetsToArray: function(forceRebuild){
                if (forceRebuild || jS.spreadsheets.length == 0) {
                    jS.cycleCellsAll(function(sheet, row, col){
                        var td = jQuery(this);
                        if(td.attr('cellvalue')){
                            jS.createCell(sheet, row, col, td.attr('cellvalue'), td.attr('formula'));
                        }else{
            ...
            controlFactory: { /* controlFactory creates the different objects requied by sheet */
                ...
                addCells: function(eq, isBefore, eqO, qty, type, skipFormulaReparse){
                        ...
                        case "col":
                            o = {
                                ...
                                dimensions: function(bar, cell, col){
                                    ...
                                    col.width(w).css('width', w + 'px').attr('width', w + 'px');
                                    
                                    bar.width(w - s.boxModelCorrection);
                ...
                barLeft: function(reloadHeights, o){
                    /* creates all the bars to the left of the spreadsheet
                    ...
                    jS.evt.barMouseDown.height(jS.obj.barLeftParent().append(barLeft));
                },
                barTop: function(reloadWidths, o){
                    /* creates all the bars to the top of the spreadsheet
                    ...
                    parents.each(function(i){
                        ...
                        var child = jQuery("<div>" + v + "</div>").width(w).height(s.colMargin);
                        barTop.append(child);
                    });
                    
                    jS.evt.barMouseDown.width(jS.obj.barTopParent().append(barTop));
                },
                barTopHandle: function(bar, i){
                    ...
                    var barTopHandle = jQuery('<div id="' + jS.id.barTopHandle + '" class="' + jS.cl.uiBarTopHandle + ' ' + jS.cl.barHelper + ' ' + jS.cl.barTopHandle + '" />').height(s.colMargin - 2).css('left', pos.left + 'px').appendTo(bar);
                ...
                barLeftHandle: function(bar, i){
                    ...
                    var barLeftHandle = jQuery('<div id="' + jS.id.barLeftHandle + '" class="' + jS.cl.uiBarLeftHandle + ' ' + jS.cl.barHelper + ' ' + jS.cl.barLeftHandle + '" />').width(s.colMargin - 6).height(s.colMargin / 3).css('top', pos.top + 'px').appendTo(bar);
                ...
                makeMenu: function(bar, menuItems){
                    ...
                    switch (bar) {
                        case "top":
                            menu = jQuery('<div id="' + jS.id.barTopMenu + '" class="'+ jS.cl.barHelper + '" />');
                            break;
                        case "left":
                            menu = jQuery('<div id="' + jS.id.barLeftMenu + '" class="'+ jS.cl.barHelper + '" />');
                            break;
                        case "cell":
                            menu = jQuery('<div id="' + jS.id.cellMenu + '" class="'+ jS.cl.barHelper + '"/>');
                            break;
                    }
                    var rightMenu = $('#rightMenu').html();
                    menu.append($('#rightMenu').html());
                    $('#rightMenu').remove();
                    menu.width(s.newColumnWidth).mouseleave(function(){
                        menu.hide();
                    }).appendTo('body').hide();
                    return menu;
                },
                ...
                header: function(){ /* creates the control/container for everything above the spreadsheet */
                    ...
                    if (jS.isSheetEditable()) {
                        ...
                        function makeMenu(ulMenu){
                            var menu = jQuery('<td id="' + jS.id.menu + '" class="' + jS.cl.menu + '" />').html(ulMenu.replace(/sheetInstance/g, "jQuery.sheet.instance[" + I + "]").replace(/menuInstance/g, I));
                            
                            menu.prependTo(firstRowTr).find("ul").hide().addClass(jS.cl.uiMenuUl).first().show();
                            
                            menu.find("li").addClass(jS.cl.uiMenuLi).hover(function(){
                                jQuery(this).find('ul:first').hide().show();
                            }, function(){
                        ...
                        jS.cl.label +
                        '"></td>' +
                        '<td style="margin:0px;width:90px;"><a href="#" title="修改报表日期"><input id="reportValue" style="width:90px;height:18px" type="text" value="" onClick="WdatePicker()"/></a></td>' +
                        ...
                        '</td>' +
                        '</tr>' +
                        '</table>').appendTo(header).find('textarea').keydown(jS.evt.keyDownHandler.formulaKeydown).keyup(function(){
                            jS.obj.inPlaceEdit().val(jS.obj.formula().val());
                        }).change(function(){
                            jS.obj.inPlaceEdit().val(jS.obj.formula().val());
                        }).bind('paste', jS.evt.pasteOverCells).focus(function(){
                            jS.setNav(false);
                        }).focusout(function(){
                            jS.setNav(true);
                        }).blur(function(){
                        ...
                        jS.setNav(true);
                        //jQuery("html")                        //.unbind('keydown') div_tabContent
                        jQuery("*[type!=text][type!=button]")                        //.unbind('keydown') div_tabContent
                        .keydown(jS.evt.keyDownHandler.documentKeydown)
                        .bind('keydown', function(e){  
                             if (sheetFocus) {//如果不是sheet掌控焦点 则跳出
                                if(e.ctrlKey   ==   true&&e.keyCode==67){
                                    if(copyTd==null){
                                        copyTd=jS.highlightedLast.td;
                                    }   
                                        //var c = jS.highlightedLast.td.attr("title");
                                       // console.info("............>"+c);
                    ...
                    var tabParent = jQuery('<div id="' + jS.id.tabContainer + '" class="' + jS.cl.tabContainer + '" />').mousedown(function(e){
                    ...
                    s.parent.html('').append(header).append('<div id="' + jS.id.ui + '" class="' + jS.cl.ui + '">').after(tabParent);
                },
                sheetUI: function(o, i, fn, reloadBars){
                    ...
                    if (jS.isSheetEditable()) {
                        ...
                        pane.mousedown(function(e){
                            var paneM = e.button == 2;
                            if (jS.isTd(e.target) && !paneM) {
                            ...
                            return false;
                        }).disableSelectionSpecial().dblclick(jS.evt.cellOnDblClick);
                    }
                ...
                table: function(){ /* creates the table control the will contain all the other controls for this instance */
                    ...
                    '<tr>' +
                    '<td id="' +
                    jS.id.barCornerParent +
                    jS.i +
                    '" class="' +
                    jS.cl.barCornerParent +
                    '">' + //corner
                    '<div style="height: ' +
                    s.colMargin +
                    '; width: ' +
                    ...
                    jS.cl.barCorner +
                    '"' +
                    (jS.isSheetEditable() ? ' onClick="jQuery.sheet.instance[' + I + '].cellSetActiveBar(\'all\');"' : '') +
                    ' title="Select All">&nbsp;</div>' +
                    '</td>' +
                    '<td class="' +
                    jS.cl.barTopTd +
                    '">' + //barTop
                ...
                safeImg: function(src, row){
                    /* creates and image and then resizes the cell's row for viewing
                     src: string, location of image;
                     row: int, the row number where the image is located;
                     */
                    return jQuery('<img />').hide().load(function(){ //prevent the image from being too big for the row
                ...
                inPlaceEdit: function(td){
                    ...
                    var h = td.height();
                    var tdval;    
                    var textarea = jQuery('<textarea id="' + jS.id.inPlaceEdit + '" class="' + jS.cl.inPlaceEdit + ' ' + jS.cl.uiInPlaceEdit + '" />').css('left', offset.left).css('top', offset.top).width(w).height(h).keydown(jS.evt.inPlaceEditOnKeyDown).keyup(function(){
                        if(tdval && textarea.val()){
                            formula.val(tdval);
                        }else{
                        ...
                        jS.setNav(true);
                    }).bind('paste', jS.evt.pasteOverCells).appendTo('body').val(function(){
                        if(tdval && formula.val()){
                            return tdval;
                        }else{
                            return formula.val();
                        }
                    ...
                    if (textarea.val() !='' && jQuery.fn.elastic) {
                        textarea.elastic();
                    }
                },
                autoFiller: function(){ /* created the autofiller object */
                    ...
                    '<div class="' +
                    jS.cl.autoFillerHandle +
                    '" />' +
                    '<div class="' +
                    jS.cl.autoFillerCover +
                    '" />' +
                    '</div>').mousedown(function(e){
                        ...
                        if (td) {
                            ...
                            jS.cellSetActive(td, loc, true, jS.autoFillerNotGroup, function(){
                                ...
                                var eLoc = jS.getTdLocation(hlighted.last());
                                //向右拖动
                                if(loc.row == eLoc.row && loc.col < eLoc.col){
                                    jS.fillRight();
                                }else{
                                ...
                                //区域拖动
                                if(hLoc.row < eLoc.row && hLoc.col < eLoc.col){
                                    jS.fillAll();
                                }
            ...
            updateCellsAfterPasteToFormula: function(oldVal){ /* oldVal is what formula should be when this is done working with all the values */
                ...
                if (loc.row == -1 && loc.col == -1) 
                    return false; //at this point we need to check if there is even a cell selected, if not, we can't save the information, so clear formula editor
                    ...
                    for (var j = 0; j < col.length; j++) {
                        ...
                        if (td.length) {
                            ...
                            if ((col[j] + '').charAt(0) == '=') { //we need to know if it's a formula here
                                ...
                                if ((col[j] + '').charAt(0) == '$') { //we need to know if it's a $ here
                                    cell.formula = col[j];
                                    td.attr('formula', col[j]);
                                    td.html(useParam(col[j]))
                                    ...
                                    td.html(col[j]).removeAttr('formula');
                                }
                            ...
                            jS.setDirty(true);
                            jS.cellSetActive(td, locNew, false);
            ...
            evt: { /* event handlers for sheet; e = event */
                keyDownHandler: {
                    ...
                    documentKeydown: function(e){
                        //console.info("sheetFocus.......>"+sheetFocus);
                        if (!sheetFocus) {//如果不是sheet掌控焦点 则跳出
                            return true;
                        }
                        if (jS.readOnly[jS.i]) 
                ...
                pasteOverCells: function(e){ //used for pasting from other spreadsheets
                    if (e.ctrlKey || e.type == "paste") {
                        ...
                        var doc = jQuery(document).one('keyup', function(){
                            fnAfter();
                            fnAfter = function(){
                            };
                            doc.mouseup();
                ...
                cellEditDone: function(forceCalc){
                    /* called to edit a cells value from jS.obj.formula(), afterward setting "fnAfterCellEdit" is called w/ params (td, row, col, spreadsheetIndex, sheetIndex)
                            ...
                            switch (jS.isFormulaEditable(td)) {
                                case true:
                                    ...
                                    if (td && jS.cellLast.row > -1 && jS.cellLast.col > -1) {
                                        ...
                                        var tdtext = v;
                                        v = v.replace(/\n/g,'<br>');
                                        var v_attr = td.attr("formula");
                                        var prevVal = td.text();
                                        var cell = jS.spreadsheets[jS.i][jS.cellLast.row][jS.cellLast.col];
                                        var v_cell=cell.formula;
                                        var isUseParam = false;
                                        var ifsingle = false;
                                        if (v.charAt(0) == '=') {
                                            if(v.indexOf('DATA') != -1 || v.indexOf('EID') != -1 || v.indexOf('ENAME') != -1 || v.indexOf('PERIODID') != -1 || v.indexOf('PERIODNAME') != -1){
                                                ifsingle = true;
                                            }
                                            td.attr('formula', v);
                                            cell.formula = v;
                                            cell.oldValue = v_cell
                                        }
                                        else 
                                            if (v.charAt(0) == '$') {
                                                td.attr('formula', v).html(useParam(v, cell));
                                                cell.value = v;
                                                cell.formula = v;
                                                isUseParam = true;
                                            }
                                            else {
                                                if(!v) td.removeAttr('cellValue').removeAttr('format');
                                                td.removeAttr('formula').html(v);
                                                if(td.attr('format') && td.attr('format') !='undefined' && v == prevVal) {
                                                    //td.attr('cellValue',v);
                                                    v =  td.attr('cellValue');
                                                }
                                                td.attr('cellValue',v);
                                                if(v && v.indexOf('<br>') != -1) td.attr('tdtext',tdtext);
                                                cell.value = v;
                                                cell.formula = null;
                                            }
                                        ...
                                        row=jS.cellLast.row;
                                        col=jS.cellLast.col;
                                        jS.updateCellValue(jS.i,jS.cellLast.row,jS.cellLast.col);
                ...
                cellEditAbandon: function(skipCalc){
                    /* removes focus of a selected cell and doesn't change it's value
                ...
                cellSetFocusFromXY: function(left, top, skipOffset){
                    /* a handy function the will set a cell active by it's location on the browser;
                ...
                cellSetFocusFromKeyCode: function(e){ /* invoke a click on next/prev cell */
                    ...
                    switch (e.keyCode) {
                        case e.altKey && key.ENTER:
                            var td=jS.highlightedLast.td;
                            var textarea = $('#'+jS.id.inPlaceEdit);
                ...
                scrollBars: function(pane){
                    /* makes the bars scroll as the sheet is scrolled
                ...
                barMouseDown: { /* handles bar events, including resizing */
                    select: function(o, e, selectFn){
                        selectFn(e.target);
                        o.unbind('mouseover').mouseover(function(e){
                            selectFn(e.target);
                        });
                        
                        jQuery(document).one('mouseup', function(){
                            o.unbind('mouseover').unbind('mouseup');
                        });
                    ...
                    height: function(o){
                        ...
                        o.unbind('mousedown').mousedown(function(e){
            ...
            isTd: function(o){
                /* ensures the the object selected is actually a td that is in a sheet
            ...
            isSheetEditable: function(i){
                i = (i == null ? jS.i : i);
                return (s.editable == true && !jS.readOnly[i]);
            },
            isFormulaEditable: function(o){
                /* ensures that formula attribute of an object is editable
            ...
            toggleFullScreen: function(){ /* toggles full screen mode */
                if (jS.obj.fullScreen().is(':visible')) { //here we remove full screen
                    ...
                    jS.obj.tabContainer().insertAfter(s.parent.append(jS.obj.fullScreen().children())).removeClass(jS.cl.tabContainerFullScreen);
                    ...
                    jS.obj.tabContainer().insertAfter(jQuery('<div class="' + jS.cl.fullScreen + ' ' + jS.cl.uiFullScreen + '" />').append(s.parent.children()).appendTo('body')).addClass(jS.cl.tabContainerFullScreen);
            ...
            tuneTableForSheetUse: function(o){
                 ...
                 */
                o.addClass(jS.cl.sheet).attr('id', jS.id.sheet + jS.i).attr('border', '1px').attr('cellpadding', '0').attr('cellspacing', '0');
            ...
            attrH: {
                ...
                setHeight: function(i, from, skipCorrection, o){
                    ...
                    if (h) {
                        jQuery(o).height(h).css('height', h + 'px').attr('height', h + 'px');
                    }
                    
                    return o;
                }
            },
            setTdIds: function(sheet, i){
                ...
                sheet.find('tr').each(function(row){
                    jQuery(this).children().each(function(col){
                        ...
                        if(td.attr('cellvalue')){
                            jS.createCell(i, row, col, td.attr('cellvalue'), td.attr('formula'));
                        }else{
            ...
            merge: function(){ /* merges cells */
                ...
                var rowI = (cellLastLoc.row - cellFirstLoc.row) + 1;
                if (rowI > 1 && colI > 1) {
                    alert(jS.msg.merge2);
                    return false;
                }
                if (rowI == colI == 1) {
                    alert(jS.msg.mergeNoChoose);
                    return false;
                    ...
                    if (cells.length > 1 /*&& cellFirstLoc.row*/) {
                        for (var i = cellFirstLoc.col; i <= cellLastLoc.col; i++) {
                            ...
                            if (i != cellFirstLoc.col) {
                                cell.formula = null;
                                cell.value;
                                td.attr('formula', '').html('').hide();
                            }
                        }
                        
                        var cell = cells.first().show().attr('colspan', colI).html(cellsValue);
                ...
                if (rowI > 1) {
                    if (cells.length > 1 && cellFirstLoc.row) {
                        for (var i = cellFirstLoc.row; i <= cellLastLoc.row; i++) {
                            var td = jQuery(jS.getTd(jS.i, i, cellFirstLoc.col)).hide();
                            var cell = jS.spreadsheets[jS.i][i][cellFirstLoc.col];
                            
                            cellsValue = (cell.formula ? "(" + cell.formula.replace('=', '') + ")" : cell.value) + cellsValue;
                            
                            if (i != cellFirstLoc.row) {
                                cell.formula = null;
                                cell.value;
                                td.attr('formula', '').html('').hide();
                            }
                        ...
                        var cell = cells.first().show().attr('rowspan', rowI).html(cellsValue);
            ...
            unmerge: function(){ /* unmerges cells */
                ...
                if (colI) {
                    ...
                    for (var i = loc.col; i < colI+loc.col; i++) {
                ...
                if (rowI) {
                    ...
                    for (var i = loc.row; i < rowI+loc.row; i++) {
                        jQuery(jS.getTd(jS.i, i, loc.col)).show();
                    }
            ...
            fillUpOrDown: function(goUp, skipOffsetForumals, v){
                ...
                var newV = v;
                var fn;
                var isKpi=v.indexOf("KPI")==1;
                var isPeriodName=v.indexOf("PERIODNAME")==1;
                var isDim=v.indexOf("ENAME")==1;
                var isData=v.indexOf("DATA")==1;
                ...
                if (v.charAt(0) == '=') {
                    fn = function(sheet, row, col){
                        ...
                        //如果是系统函数就调用重写的reparseFormula方法
                        
                        if(isKpi || isPeriodName || isDim){
                            
                            newV = jS.reparseFormula_period(v, offset);
                            cell.formula = newV;
                            
                            td.attr('formula', newV).html('');
                            
                            if(isKpi){
                                computeFormula_kpi(null,"kpi",cell,jQuery(jS.getTd(sheet, row, col)));
                            } else if(isPeriodName){
                                cell.calcCount = 0;
                    ...
                    fn = function(sheet, row, col){
                        ...
                        //checkDate('2011-11-31','yyyy-MM-dd','no right')
                        if(checkDate(newV,'yyyy-MM-dd','')){
                            newV=addDate(newV,1,'-')
                    }   
                    };
                }
            ...
            reparseFormula_period: function(formula, offset){
                var isPeriodName=formula.indexOf("PERIODNAME")==1;
                var isKpi=formula.indexOf("KPI")==1;
                var isDim=formula.indexOf("ENAME")==1;
                //var isData=formula.indexOf("DATA")==1;
                var formula_=parseFormula(formula);
                var fors=formula_.split(",");
                if(isPeriodName){
                    if(fors.length == 3){
                        var varNum=new Number(fors[2]);
                        var newres=varNum+offset.row;
                        return "=PERIODNAME(\""+fors[0]+"\","+"\""+fors[1]+"\","+"\""+newres+"\")"
                    }else{
                        if(offset.row==0){
                            return formula;
                        }else{
                            return "=PERIODNAME(\""+fors[0]+"\","+"\""+fors[1]+"\","+"\""+offset.row+"\")";
                        }   
                    }   
                }
                
                if(isKpi){
                    var isDimInKpi=formula.indexOf("EID")!=-1;
                    var reg="";
                    if(isDimInKpi){
                        reg=/EID\(\"(.*)\",(.*)\),/;
                    }else{
                    ...
                    var formulas=formula.split('KPI');
                    var newV='';
                    var result='';
                    var num='';
                    var newres='';
                    ...
                    for(i=1;i<formulas.length;i++){
                        if(reg.test(formulas[i])){
                            if(isDimInKpi){
                                result=reg.exec(formulas[i]);
                                num=Number(result[2].valueOf());
                                newres=num+offset.row;
                                dim=result[1];
                                formulas[i]='KPI'+formulas[i].replace(reg,'EID(\"'+dim+'\",'+newres+'),');
                            }else{
                                result=reg.exec(formulas[i]);
                                num=Number(result[1].valueOf());
                                newres=num+offset.row;
                                if(newres>=0){
                                    formulas[i]='KPI'+formulas[i].replace(reg,'\"\)+'+newres+',');
                                }else{
                                    formulas[i]='KPI'+formulas[i].replace(reg,'\"\)'+newres+',');
                                }
                            }
                            ...
                            if(offset.row==0){
                                formulas[i]='KPI'+formulas[i];
                            }else{
                                formulas[i]='KPI'+formulas[i].replace(reg,'+'+offset.row+',');
                            }   
                        }
                        newV+=formulas[i];
                    }
                ...
                if(isDim){
                    var num=Number(fors[1]);
                    var newres=num+offset.row;
                    return "=ENAME(\""+fors[0]+"\","+"\""+newres+"\")";
                }
                return formula;
             }, 
             
             //向右拖动
             fillRight: function(v){
                 /* fills values down or up to highlighted cells from active cell;
                  goUp: bool, default is down, when set to true value are filled from bottom, up;
                  skipOffsetForumals: bool, default is formulas will offest, when set to true formulas will stay static;
                  v: string, the value to set cells to, if not set, formula will be used;
                 ...
                 if (v.charAt(0) == '=') {
                     fn = function(sheet, row, col){
                         ...
                         var cell = jS.spreadsheets[sheet][row][col];
                         if(isKpi || isPeriodName || isDim ){
                            newV = jS.reparseFormula_right(v, offset);
                            jS.spreadsheets[sheet][row][col].formula = newV;
             ...
             reparseFormula_right: function(formula, offset){
                var isPeriodName=formula.indexOf("PERIODNAME")==1;
                var isKpi=formula.indexOf("KPI")==1;
                var isDim=formula.indexOf("ENAME")==1;
                ...
                if(isPeriodName){
                    if(fors.length == 3){
                        var varNum=new Number(fors[2]);
                        var newres=varNum+offset.col;
                        return "=PERIODNAME(\""+fors[0]+"\","+"\""+fors[1]+"\","+"\""+newres+"\")"
                    }else{
                        if(offset.col==0){
                            return formula;
                        }else{
                            return "=PERIODNAME(\""+fors[0]+"\","+"\""+fors[1]+"\","+"\""+offset.col+"\")";
                        }   
                    }   
                }
                    ...
                    for(i=1;i<formulas.length;i++){
                        if(reg.test(formulas[i])){
                            ...
                            if(offset.col==0){
                                ...
                                formulas[i]='KPI'+formulas[i].replace(reg,'+'+offset.col+',');
                            }   
                        }
                        newV+=formulas[i];
                    }
            ...
            fillAll: function(v){
                  ...
                  if (v.charAt(0) == '=') {
                      fn = function(sheet, row, col){
                        ...
                        vLeft=jS.spreadsheets[sheet][row][startLoc.col-1].formula;
                        vLeft=vLeft.replace("PERIODNAME","PERIODID");
                        var lefts=vLeft.split(",");
                        if(lefts.length == 3){
                            vLeft=lefts[0]+","+lefts[1]+")+"+lefts[2].replace("\)","");
                        }
                        vUp=jS.spreadsheets[sheet][startLoc.row-1][col].formula;
                        vUp=vUp.replace("ENAME","EID");
                        
                        newV = "=KPI("+vLeft+","+vUp+","+v.split(",")[4];
                        jS.spreadsheets[sheet][row][col].formula = newV;
            ...
            offsetFormulas: function(loc, offset, isBefore){
                /* makes cell formulas increment in a range
                 loc: {row: int, col: int}
              ...
              //增加行列时处理应用关系map
                ...
                if(map.size() > 0){
                    for(var i=0;i<map.size();i++){
                        ...
                        var key=m.key();
                        var value=m.value();
                        key=key.replace(jSE.regEx.cell,function(ref){
                            var keyloc=jSE.parseLocation(ref);
                            
                            var keyrow=isInFormula(keyloc.row, loc.row);
                            var keycol=isInFormula(keyloc.col, loc.col);
                            if (keycol) 
                                keyloc.col += offset.col;
                            if (keyrow) 
                                keyloc.row += offset.row;
                            if(/\$/.test(ref)) return parseRef(ref,offset,keyloc,false);
                            return jS.makeFormula(keyloc);
                        });
                        value=value.replace(jSE.regEx.cell,function(ref){
                            var valloc=jSE.parseLocation(ref);
                            
                            var valrow=isInFormula(valloc.row, loc.row);
                            var valcol=isInFormula(valloc.col, loc.col);
                            if (valcol) 
                                valloc.col += offset.col;
                            if (valrow) 
                                valloc.row += offset.row;
                            if(/\$/.test(ref)) return parseRef(ref,offset,valloc,false);
                            return jS.makeFormula(valloc);
                        });
                ...
                jS.cycleCells(function(sheet, row, col){
                    ...
                    if (formula && jS.isFormulaEditable(td)) {
                        var formula_=formula.replace(/=/,"");
                        if(!isSysFormula(formula_)){
                ...
                return formula.replace(jSE.regEx.cell, function(ignored, col, row, pos){
                    ...
                    if (fn) {
                        ...
                        if (move.col || move.row) {
                            ...
                            if (move.row) 
                                loc.row += offset.row;
                            if(/\$/.test(ignored)) return parseRef(ignored,offset,loc,false);
                        ...
                        if(/\$/.test(ignored)) return parseRef(ignored,offset,loc,true);
            ...
            cycleCells: function(fn, firstLoc, lastLoc, sheet){
                /* cylces through a certain group of cells in a spreadsheet and applies a function to them
            ...
            cycleCellsAndMaintainPoint: function(fn, firstLoc, lastLoc){
                /* cylces through a certain group of cells in a spreadsheet and applies a function to them, firstLoc can be bigger then lastLoc, this is more dynamic
                 fn: function, the function to apply to a cell;
                 firstLoc: array of int - [col, row], the group to start;
                 lastLoc: array of int - [col, row], the group to end;
                 */
            ...
            addTab: function(){ /* Adds a tab for navigation to a spreadsheet */
                ...
                jS.sheetTab(true) +
                '</a>' +
                '</span>').insertBefore(jS.obj.tabContainer().find('span:last'));
            },
            sheetDecorate: function(o){
                /* preps a table for use as a sheet;
                 o: object, table object;
                 */
                jS.formatSheet(o);
            ...
            formatSheet: function(o){
                ...
                if (o.find('colgroup').length < 1 || o.find('col').length < 1) {
                    ...
                    o.find('tr:first').children().each(function(){
                        var w = s.newColumnWidth;
                        jQuery('<col />').width(w).css('width', (w) + 'px').attr('width', (w) + 'px').appendTo(colgroup);
                        
                        tableWidth += w;
                    });
                    o.find('tr').each(function(){
                        jQuery(this).height(s.colMargin).css('height', s.colMargin + 'px').attr('height', s.colMargin + 'px');
                    });
                    colgroup.prependTo(o);
                }
                
                o.removeAttr('width').css('width', '').width(tableWidth);
            },
            ...
            themeRoller: { /* jQuery ui Themeroller integration */
                ...
                cell: {
                    setActive: function(){
                        this.clearActive();
                        this.setHighlighted(jS.cellLast.td.addClass(jS.cl.cellActive));
                    },
                    setHighlighted: function(td){
                        jQuery(td).addClass(jS.cl.cellHighlighted + ' ' + jS.cl.uiCellHighlighted);
                    },
                    clearActive: function(){
                        jS.obj.cellActive().removeClass(jS.cl.cellActive);
                    },
                    ...
                    clearHighlighted: function(){
                        if (jS.themeRoller.cell.isHighlighted()) {
                            jS.obj.cellHighlighted().removeClass(jS.cl.cellHighlighted + ' ' + jS.cl.uiCellHighlighted);
                        }
                ...
                bar: {
                    ...
                    clearActive: function(){
                        jS.obj.barTop().add(jS.obj.barLeft()).children('.' + jS.cl.uiActive).removeClass(jS.cl.uiActive);
                    }
                },
                tab: {
                    ...
                    clearActive: function(){
                        jS.obj.tabContainer().find('span.' + jS.cl.uiTabActive).removeClass(jS.cl.uiTabActive);
                    }
                },
            ...
            resizable: function(o, settings){
                ...
                o.resizable(settings).attr('resizable', true);
            },
            busy: false,
            draggable: function(o, settings){
                ...
                o.draggable(settings).attr('draggable', true)
            },
            resizeBarTop: function(e){
                jS.resizable(jQuery(e.target), {
                    ...
                    stop: function(e, ui){
                        ...
                        jS.obj.sheet().find('col').eq(i).width(w).css('width', w + 'px').attr('width', w + 'px');
            ...
            sheetDecorateRemove: function(makeClone){
                ...
                jQuery(o).find('td.' + jS.cl.cellActive).removeClass(jS.cl.cellActive + ' ' + jS.cl.uiCellActive);
                
                jQuery(o).find('td.' + jS.cl.cellHighlighted).removeClass(jS.cl.cellHighlighted + ' ' + jS.cl.uiCellHighlighted);
                /*
            ...
            cellEdit: function(td, isDrag, skipFocus){
                /* starts cell to be edited
                 td: object, td object;
                ...
                var formula = jS.obj.formula().val(v).blur();
            ...
            cellSetActive: function(td, loc, isDrag, directional, fnDone){
                /* cell cell active to sheet, and highlights it for the user, shouldn't be called directly, should use cellEdit
                 td: object, td object;
                ...
                if (typeof(loc.col) != 'undefined') {
                    ...
                    if (isDrag) {
                        ...
                        jS.obj.pane().mousemove(function(e){
                        ...
                        jQuery(document).one('mouseup', function(){
                            ...
                            jS.obj.pane().unbind('mousemove').unbind('mouseup');
            ...
            updateCellValue: function(sheet, row, col){
                ...
                var cell = jS.spreadsheets[sheet][row][col];
                var td = jQuery(jS.getTd(sheet, row, col));
                cell.state = "red";
                
                var isSysFormula_=false;//是否是系统公式
                var isNoCheckSysFormula=true;//是否循环检查系统公式(只检查高亮显示的单元格的系统公式) : 默认不检查
                
                var isRight=false;
                var fl='';
                if(cell.formula){
                    fl= cell.formula;
                    if(fl.indexOf('=') != -1) fl=fl.replace('=','');
        
                    /*处理跨sheet引用*/
                    if(fl.match(jSE.regEx.remoteCell)){
                        fl = jS.updateRemoteFormula(fl);
                        cell.formula = fl;
                    }
                }
                ...
                var isInitSheet=!sheetFocus&&jS.highlightedLast.colStart==-1&&jS.highlightedLast.rowStart==-1;
                var isCurHighlightTd=jS.cellLast.td.length!=0 && row >= hLoc.row && row <= eLoc.row && col >= hLoc.col && col <= eLoc.col;
                var Parser = getParser(jS,cell,sheet, row, col, s);
                
                //处理单元格换行
                if(td.html() && td.html().indexOf('<br>') != -1) td.attr('cellvalue',td.html());
                
                //特殊处理历史遗留报表问题单元格数据1.ENAME PERIODNAME返回不是id 2.cell.value为格式化后的值
                
                if(fl.indexOf('ENAME') == 0 || fl.indexOf('PERIODNAME') == 0) {
                    ifsys = true;
                    sysfor = true;
                    if(!/^\d+$/.test(cell.value)){
                        cell.value = Parser.parse(fl);
                    }
                    td.attr('cellvalue',cell.value);
                }
                var curCellVal=cell.value.toString();
                if(fl && curCellVal && curCellVal.indexOf('%') != -1){
                    cell.value = Parser.parse(fl);
                    td.attr('cellvalue',cell.value);
                }
                //end
                    ...
                    if (cell.formula) {
                        try {
                            if (cell.formula.charAt(0) == '=') {
                                ...
                                //cell.formula=paserParam(cell.formula);
                            }
                            var isKPI=cell.formula.indexOf("KPI")==0;
                            //var isPeriod=cell.formula.indexOf("PERIODID")==0;
                            var isPeriod = false;
                            //var isPeriodName=cell.formula.indexOf("PERIODNAME")==0;
                            var isPeriodName=false;
                            var isDim=false;
                            //var isDim=cell.formula.indexOf("ENAME")==0;
                            //var isData=cell.formula.indexOf("DATA")==0;
                            var isData = false;
                            
                            if (isSysFormula(cell.formula)) {//是否循环检查系统公式
                                isSysFormula_=true;
                                try {
                                    computeFormula_kpi(null,"kpi",cell,jQuery(jS.getTd(sheet, row, col)));
                                    isKPI=true;
                            ...
                            //如果是系统公式并且要检查，或者不是系统公式 则 ...
                            if(!isSysFormula_){
                                doParser(cell,sheet,row,col,s);
                                formatRes(cell,sheet,row,col,s);
                            }
                        } 
                        catch (e) {
                ...
                }else if ((!forceref && isCurHighlightTd && cell.calcCount < 1)){
                    cell.calcLast = jS.calcLast;
                    cell.calcCount++;
                    if (cell.formula) {
                        try {
                            ...
                            //alert(isSysFormula(cell.formula));
                            if (isSysFormula(cell.formula)) {//是否循环检查系统公式
                                isSysFormula_=true;
                                try {
                                    if(!isInitSheet){//初始化不进行执行查询
                                        if((isInitSheet&&isKPI)||(isKPI&&isCurHighlightTd)){
                                            computeFormula_kpi(null,"kpi",cell,jQuery(jS.getTd(sheet, row, col)));
                                            isKPI=true;
                                        }else if((isInitSheet&&isPeriod)||(isPeriod&&isCurHighlightTd)){
                                            computeFormula(null,"id",cell,jQuery(jS.getTd(sheet, row, col)));
                                        }else if((isInitSheet&&isPeriodName)||(isPeriodName&&isCurHighlightTd)){
                                            computeFormula(null,"period",cell,jQuery(jS.getTd(sheet, row, col)));
                                        }else if((isInitSheet&&isDim)||(isDim&&isCurHighlightTd)){
                                            computeFormula_dim(null,"dim",cell,jQuery(jS.getTd(sheet, row, col)));
                                        }else if((isInitSheet&&isData)||(isData&&isCurHighlightTd)){
                                            computeFormula_data(null,"data",cell,jQuery(jS.getTd(sheet, row, col)));
                                        }
                                    }
                            ...
                            //如果是系统公式并且要检查，或者不是系统公式 则 ...
                            if(!isSysFormula_){
                                var args = doParser(cell,sheet,row,col,s);
                                formatRes(cell,sheet,row,col,s);
                                //检查当前编辑单元格是否被引用
                                var key=jSE.columnLabelString(col)+(row+1);
                                cellRec='';//清空递归引用
                                var refcells = getRecursiveRef(key);
                                if((isCurHighlightTd && refcells && !forceref)){    
                                    var cellstoref=refcells.split(',');
                                    for(var i=0;i<cellstoref.length;i++){
                                        var loc = jSE.parseLocation(cellstoref[i]);
                                        doParser(null,sheet,loc.row,loc.col,s);
                                        formatRes(null,sheet,loc.row,loc.col,s);
                                    }
                                }
                            }
                        } 
                ...
                if(isSysFormula_){
                    return jQuery(jS.getTd(sheet, row, col)).html().trim();
                }else{
                    return cell.value;
                } 
            },
            updateRemoteFormula: function(formula){
                var remoteCellMatch = formula.match(jSE.regEx.remoteCell)[0];
                var remoteSheet = remoteCellMatch.split(':')[0];
                var remoteId = remoteCellMatch.split(':')[1];
                var remoteValue = jS.cellHandlers.remoteCellValue(remoteSheet,remoteId)
                formula = formula.replace(jSE.regEx.remoteCell,remoteValue)
                return formula;
            },
            cellHandlers: {
                cellValue: function(id){ //Example: A1
                    var loc = jSE.parseLocation(id);
                    console.info("cellHandlers---------------cellValue")            
                ...
                cellRangeValue: function(start, end){//Example: A1:B1
                    ...
                    for (var i = start.row; i <= end.row; i++) {
                        for (var j = start.col; j <= end.col; j++) {
                            console.info("cellHandlers---------------cellRangeValue");
                ...
                remoteCellValue: function(sheet, id){//Example: SHEET1:A1
                    ...
                    console.info("cellHandlers---------------remoteCellValue")
                ...
                remoteCellRangeValue: function(sheet, start, end){//Example: SHEET1:A1:B2
                    ...
                    for (var i = start.row; i <= end.row; i++) {
                        for (var j = start.col; j <= end.col; j++) {
                            console.info("cellHandlers---------------remoteCellRangeValue")
            ...
            context: {},
            calcLast: 0,
            calc: function(tableI){
                /* harnesses calculations engine's calculation function
                ...
                if (jS.readOnly[tableI]) 
                    return; //readonly is no calc at all
                jS.log('Calculation Started');
                jS.calcLast = new Date();
                jSE.calc(tableI, jS.spreadsheetsToArray(true)[tableI], jS.updateCellValue);
            ...
            deleteRow: function(skipCalc){ /* removes the currently selected row */
                if(jS.obj.sheet().find('tr').length==1){
                    alert('亲,仅剩一行了，不能删除了')
                    return false;
                }
            ...
            deleteColumn: function(skipCalc){ /* removes the currently selected column */
               if( jS.obj.sheet().find('colgroup col').length==1){
                    alert('亲,仅剩一列了，不能删除了')
                    return false;
                }               
            ...
            sheetTab: function(get){
                ...
                if (get) {
                    ...
                    if (jS.isSheetEditable() && s.editableTabs) { //ensure that the sheet is editable, then let them change the sheet's name
            ...
            HTMLtoPrettySource: function(node, prefix){
                /* prints html to manu lines, formatted for easy viewing
                 node: object;
                 prefix: string;
                 */
                if (!prefix) {
            ...
            followMe: function(td){
                ...
                if ((tdPos.left + tdWidth + margin) > (panePos.left + paneWidth)) { //right
                    ...
                    if (tdPos.left < panePos.left) { //left
                        pane.stop().scrollTo(td, {
                            axis: 'x',
                            duration: 50
                        });
            ...
            autoFillerGoToTd: function(td, tdHeight, tdWidth){
                ...
                if (s.autoFiller) {
                    if (td.attr('id')) { //ensure that it is a usable cell
                        tdPos = td.position();
                        jS.obj.autoFiller().show().css('top', ((tdPos.top + (tdHeight ? tdHeight : td.height()) - 3) + 'px')).css('left', ((tdPos.left + (tdWidth ? tdWidth : td.width()) - 3) + 'px'));
                    }
                }
            },
            isRowHeightSync: [],
            ...
            openSheetURL: function(url){
                /* opens a table object from a url, then opens it
                 url: string, location;
                 */
                s.urlGet = url;
                return jS.openSheet();
            ...
            importRow: function(rowArray){
                /* creates a new row and then applies an array's values to each of it's new values
                 rowArray: array;
                 */
            ...
            exportSheet: { /* exports sheets into xml, json, or html formats */
                xml: function(skipCData){
                    ...
                    jQuery(sheetClone).each(function(){
                        ...
                        '<columns>' +
                        colCount +
                        '</columns>' + //length is 1 based, index is 0 based
                        '<rows>' +
                        rowCount +
                        '</rows>' + //length is 1 based, index is 0 based
                        '<col_widths>' +
                        col_widths +
                        '</col_widths>' +
                        '</metadata>' +
                    ...
                    jQuery(sheetClone).each(function(){
                        ...
                        trs.each(function(i){
                            ...
                            tds.each(function(j){
                                ...
                                var tdbr=td.attr('cellvalue');
                                var text=td.text();
                                var tdtext=td.attr('tdtext');
                                if(tdbr && tdbr.indexOf('<br>') != -1){
                                    text = tdbr;
                                }
                                document['data']['r' + i]['c' + j] = {
                                    'value': (formula ? formula : text),
                                    'text': td.text(),
                                    'style': td.attr('style'),
                                    'colspan': colSpan,
                                    'rowspan': rowSpan,
            ...
            sheetSyncSizeToDivs: function(){ /* syncs a sheet's size from bars/divs */
                ...
                jS.obj.sheet().width(newSheetWidth).attr('width', newSheetWidth + 'px').css('width', newSheetWidth + 'px');
                return newSheetWidth;
            },
            sheetSyncSizeToCols: function(o){
                /* syncs a sheet's size from it's col objects
            ...
            sheetSyncSize: function(){ /* syncs a sheet's size to that of the jQuery().sheet() caller object */
                ...
                var mainH = screen.height - 270;
                $('#jQuerySheetMy').css('height',mainH+'px');
                var h = mainH;
                if (!h) {
                ...
                s.parent.height(h).width(s.width);
                ...
                jS.obj.pane().height(h).width(w).parent().width(w);
                
                jS.obj.ui().width(w + jS.attrH.width(jS.obj.barLeftParent()));
                
                jS.obj.barLeftParent().height(h);
                
                jS.obj.barTopParent().width(w).parent().width(w);
            },
            ...
            getTdId: function(tableI, row, col){
                /* makes a td if from values given
            ...
            getTdLocation: function(td){
                ...
                return {
                    ...
                    row: td[0].parentNode ? parseInt(td[0].parentNode.rowIndex) : 0
                }
            },
            ...
            cellUndoable: {
                ...
                undoOrRedo: function(undo){
                    ...
                    this.get().clone().each(function(){
                        ...
                        if (id) {
                            jQuery('#' + id).replaceWith(o.removeAttr('undoable').attr('id', id));
                        }
                        else {
                ...
                add: function(tds){
                    var oldTds = tds.clone().each(function(){
                        ...
                        .attr('undoable', id).removeClass(jS.cl.cellHighlighted + ' ' + jS.cl.uiCellHighlighted);
                    });
        ...
        s.parent.html('').addClass(jS.cl.parent);
        
        origParent.unbind('switchSpreadsheet').bind('switchSpreadsheet', function(e, js, i){
            jS.switchSpreadsheet(i);
        }).unbind('renameSpreadsheet').bind('renameSpreadsheet', function(e, js, i){
    ...
    makeTable: {
        xml: function(data){
            /* creates a table from xml, note: will not accept CDATA tags
            ...
            jQuery(data).find('document').each(function(i){ //document
                ...
                table.width(tableWidth).attr('title', title);
                ...
                table.append(tbody).appendTo(tables);
            });
            
            return tables.children();
        },
        json: function(data, makeEval){
            /* creates a sheet from json data, for format see top
             data: json;
            ...
            for (var i = 0; i < sheet.length; i++) {
                ...
                table.attr('title', title).width(tableWidth);
                ...
                for (var x = 0; x < rowCount; x++) { //tr
                    ...
                    for (var y = 0; y < colCount; y++) { //td
                        ...
                        if (cell) {  
                            cur_val = cell.value + '';
                            cur_text = cell.text;
                            colSpan = cell.colSpan == null ? cell.colspan : cell.colSpan + '';
                            rowspan = cell.rowspan == null ? cell.rowspan : cell.rowspan + '';
                            style = cell.style + '';
                            cl = cell.cl + '';
                            title = cell.title==null?"":cell.title + '';
                            cur_format = cell.format == null ? cell.format : cell.format + '';
                            tdtext = cell.tdtext == null ? cell.tdtext : cell.tdtext + '';
                        }
                        ...
                        try {
                            ...
                            if (typeof(cur_val) == "number") {
                                //cur_td.html(cur_val);
                                cur_td.html(cur_format);
                            }
                            else {
                                if(cell.formula != null &&cell.formula.charAt(0) == "="){//由后台传过来的formula
                                     cur_td.attr("formula", cell.formula);
                                     if(cur_val.indexOf("=KPI")==0||cur_val==""){
                                        cur_td.html(reportInfo.defHtml);
                                        cur_td.attr("title",cell.kpiNoResultData);
                                     }else{
                                         cur_td.html(cur_val);
                                     }
                                }else if (cur_val.charAt(0) == '=') {
                                    cur_td.attr("formula", cur_val);
                                    cur_td.html(cur_text);
                                }
                                else {
                                    cur_td.html(cur_val);
                                }
    ...
    paneScrollLocker: function(e, jS){ //This can be used with setting fnPaneScroll to lock all loaded sheets together when scrolling, useful in history viewing
        ...
        jQuery(jQuery.sheet.instance).each(function(i){
            ...
            this.obj.pane().scrollLeft(pane.scrollLeft()).scrollTop(pane.scrollTop());
        });
    },
...
var jSE = jQuery.sheet.engine = { //Calculations Engine
    ...
    parseLocation: function(locStr){ // With input of "A1", "B4", "F20", will return {row: 0,col: 0}, {row: 3,col: 1}, {row: 19,col: 5}.
        ...
        var reg=/\$/g;
        if(reg.test(locStr)) {
            var colStr = locStr.match(/[A-Za-z]+/);
            var rowStr = locStr.match(/\d+/);
             return {
                    row: parseInt(rowStr) - 1,
                    col: jSE.columnLabelIndex(colStr.toString())
                };
        }
    ...
    columnLabelIndex: function(str) {
        ...
        if(str.length == 1){
            var digit = str.toUpperCase().charCodeAt(0) - 65;      // 65 == 'A'.
            num = digit;
            ...
            for(var i = 0 ; i< str.length ; i++){
                ...
                if(i == 0) {
                    first_digit = (digit+1) * 26
                digit = 0;
                }
                num += digit + first_digit;  
            }
        }
    ...
    cFN: {//cFN = compiler functions, usually mathmatical
        ...
        clean: function(v){
            if (typeof(v) == 'string') {
                v = v.replace(jSE.regEx.amp, '&').replace(jSE.regEx.nbsp, ' ').replace(/\n/g, '').replace(/\r/g, '');
            }
            return v;
        },
        sanitize: function(v){
    ...
    chart: function(o){
        /* creates a chart for use inside of a cell
        ...
        o = jQuery.extend({
            ...
            data: [0],
            legend: "",
            chart: jQuery('<div class="' + jS.cl.chart + '" />').mousedown(function(){
        ...
        this.s.origParent.one('calculation', function(){
            ...
            if (o.title) 
                r.text(width / 2, 10, o.title).attr({
                    "font-size": 20
                });
            switch (o.type) {
                case "bar":
                    r.barchart(width / 8, height / 8, width * 0.8, height * 0.8, o.data, o.legend).hover(function(){
                    ...
                    break;
                case "hbar":
                    r.hbarchart(width / 8, height / 8, width * 0.8, height * 0.8, o.data, o.legend).hover(function(){
                        this.flag = r.popup(this.bar.x, this.bar.y, this.bar.value || "0").insertBefore(this);
                    }, function(){
                        this.flag.animate({
                            opacity: 0
                ...
                case "line":
                    ...
                    break;
                case "pie":
                    var pie = r.piechart(width / 2, height / 2, (width < height ? width : height) / 2, o.data, {
                        legend: o.legend
                    }).hover(function(){
                        this.sector.stop();
...
var jFN = jQuery.sheet.fn = {//fn = standard functions used in cells
    USERBASE: function(){
        jQuery.ajax({
            url: "http://localhost:8080/datavalue/recommend/result",
            type: 'get',
            dataType: 'jsonp',
            success: function(data){
                var selectedTd = jS.highlightedLast.td,
                    selectedLoc = {
                ...
                for(var userId in data){
                    var tdSource = jQuery(jS.getTd(jS.i, rowIndex+selectedLoc.row, selectedLoc.col));
                    tdSource.html(userId);
                    var recommeds = data[userId];
                    for(var i=1;i<=recommeds.length;i++){
                        var tdRecommed = jQuery(jS.getTd(jS.i, rowIndex+selectedLoc.row, i+selectedLoc.col));
                        tdRecommed.html(recommeds[i-1]);
                         if (i == jS.sheetSize().width) {
                            jS.controlFactory.addColumn(':last');
                        }
                    }
                    rowIndex++;
                    if (rowIndex == jS.sheetSize().height) {
                        jS.controlFactory.addRow(':last');
                    }
                }
            }
    ...
    DATA: function(){
        var cell=this.cell;
        var td = jQuery(this.jS.getTd(this.sheet, this.row, this.col));
        var vs= arguments;
        
        return computeFormula_data(vs,cell,td,false);
    },
    PERIODID: function(){
        var cell=this.cell;
        var td = jQuery(this.jS.getTd(this.sheet, this.row, this.col));
    ...
    ENAME: function(){
        ...
        return parseEname(vs,cell,td);
    },
    VERSION: function(){
        return this.jS.version;
    },
    IMG: function(v){
        return jQuery('<img />').attr('src', v);
    },
    ...
    DROPDOWN: function(v, noBlank){
        ...
        if (this.s.editable) {
            ...
            var o = jQuery('<select style="width: 100%;" name="' + id + '" id="' + id + '" />').mousedown(function(){
    ...
    RADIO: function(v){
        ...
        if (this.s.editable) {
            ...
            var o = jQuery('<span />').mousedown(function(){
                jS.cellEdit(jQuery(this).parent());
            });
            
            for (var i = 0; i < (v.length <= 25 ? v.length : 25); i++) {
                if (v[i]) {
                    var input = jQuery('<input type="radio" name="' + id + '" class="' + id + '" />').val(v[i]);
                    
                    if (v[i] == cell.selectedValue) {
                        input.attr('checked', 'true');
                    }
                    
                    o.append(input).append('<span>' + v[i] + '</span>').append('<br />');
    ...
    CHECKBOX: function(v){
        ...
        if (this.s.editable) {
            ...
            var checkbox = jQuery('<input type="checkbox" name="' + id + '" class="' + id + '" />').val(v);
            
            var o = jQuery('<span />').append(checkbox).append('<span>' + v + '</span><br />').mousedown(function(){
                jS.cellEdit(jQuery(this).parent());
            });
    ...
    CALCTIME: function(){
        var owner = this;
        this.s.origParent.one('calculation', function(){
            jQuery(owner.jS.getTd(owner.sheet, owner.row, owner.col)).text(owner.jS.time.diff());
        });
        return "";
    }
};
